...
for script_name in script_names:
    ...
    os.system(f"python {file_path}/{script_name} --gpus {args.gpus}{use_wandb} --wandb_entity {args.wandb_entity} --model MusGConv --use_reledge --pitch_embedding 16 --return_edge_emb")

    # The MusGConv model with signed edge features
    os.system(f"python {file_path}/{script_name} --gpus {args.gpus}{use_wandb} --wandb_entity {args.wandb_entity} --model MusGConv --use_reledge --pitch_embedding 16 --use_signed_features")

    # The MusGConv model without pitch embedding
    os.system(f"python {file_path}/{script_name} --gpus {args.gpus}{use_wandb} --wandb_entity {args.wandb_entity} --model MusGConv --use_reledge")

    # The MusGConv model without edge features (i.e. reledge set to False)
    os.system(f"python {file_path}/{script_name} --gpus {args.gpus}{use_wandb} --wandb_entity {args.wandb_entity} --model MusGConv")

    # NOTE: Missing the MusGConv without concatenation of the edge features (Equivariance argument).